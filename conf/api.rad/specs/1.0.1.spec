...
if [ "$1" = "1" ]; then
  ...
  php /var/www/sites/rad/api/init/install.sh
  
  # Copy the logrotate.d so that we can rotate our logs
  cp -f /var/www/sites/rad/api/init/config/logrotate /etc/logrotate.d/api.rad
  
  # Copy the crontab so that we can start our processes
  cp -f /var/www/sites/rad/api/init/config/crontab /etc/cron.d/rad
elif [ "$1" = "2" ]; then
  ...
  # Copy the crontab so that we can start our processes
  rm -f /etc/cron.d/rad
  cp -f /var/www/sites/rad/api/init/config/crontab /etc/cron.d/api.rad
fi
